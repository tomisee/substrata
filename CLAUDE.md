...
- [ ] **TCP handshake** — stubbed as `planned`; build next (SYN / SYN-ACK / ACK, sequence numbers, state on both ends). Use `CacheSim.tsx` as the structural template.
- [ ] Floating-point rounding visualizer
- [ ] The stack frame during a function call
- [ ] Animate chip layers sliding apart on toggle (currently just show/hide)
- [ ] More accurate die proportions / flip-chip bump layout
- [ ] Replace placeholder GitHub link in `app/layout.tsx`
